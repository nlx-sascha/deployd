...
Collection.events  = ['Get', 'Validate', 'Post', 'Put', 'Commit', 'Delete'];
    ...
    store.first(sanitizedQuery, function(err, obj) {
      ...
      function commit(err) {
        ...
        store.update({id: query.id}, item, function (err) {
          ...
          if(collection.shouldRunEvent(collection.events.Commit, ctx)) {
            collection.events.Commit.run(ctx, domain, function() {
              done(null, item);
            });
          } else {
            done(null, item);
